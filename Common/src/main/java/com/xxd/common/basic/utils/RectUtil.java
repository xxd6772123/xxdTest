...
public class RectUtil {
    ...
    /**
     * 将传入的矩形区域进行对应的宽度和高度的缩放后得到新的矩形区域
     *
     * @param rect
     * @param scaleW
     * @param scaleH
     * @return
     */
    public static RectF scaleRect(RectF rect, float scaleW, float scaleH) {
        if (rect == null) {
            return null;
        }
        RectF newRect = new RectF();
        newRect.left = rect.left * scaleW;
        newRect.right = rect.right * scaleW;
        newRect.top = rect.top * scaleH;
        newRect.bottom = rect.bottom * scaleH;
        return newRect;
    }
