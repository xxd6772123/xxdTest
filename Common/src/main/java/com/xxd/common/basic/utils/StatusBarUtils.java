 ...
 * @time:2022/8/14
 * @desc:
 * //清单文件中要设置主题为NoActionBar 。使用：设置statusBar颜色为蓝色
 *  * StatusBarUtils.with(this)
 *  * .setColor(getResources().getColor(R.color.blue))
 *  * .init();
 *  * <p>
 *  * //清单文件中要设置主题为NoActionBar
 *  * //调用之后当前界面的layout将为沉浸式状态栏布局，控件从（0，0）而非（0，statusBarHeight）开始
 *  * StatusBarUtils.with(this)
 *  * .init();
 *  * <p>
 *  * 更多请参考：https://blog.csdn.net/yzwfeng/article/details/100040067
 */
...
//                attributes.flags |= flagTranslucentNavigation;
    ...
    //true 黑色  //false 白色
    public static void setSystemStatusTextColor(boolean setDark, Activity activity) {
    ...
    public static void setStatusBarTranslate(Activity activity) {
        setStatusBarTranslate(activity, View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
    }

    /**
     * @param value View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR 状态栏字体黑
     *              SYSTEM_UI_FLAG_LIGHT_NAVIGATION_BAR状态栏字体白
     */
    public static void setStatusBarTranslate(Activity activity, int value) {
        if (Build.VERSION.SDK_INT >= 21) {
            Window window = activity.getWindow();
    ...
    private static void setFlag(Window window, int flag) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            View decorView = window.getDecorView();
    ...
    private static void clearFlag(Window window, int flag) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            View decorView = window.getDecorView();
            int option = decorView.getSystemUiVisibility() & (~flag);
            decorView.setSystemUiVisibility(option);
    ...
    public static void setStatusBarVisibility(@NonNull Window window, boolean isVisible) {
        if (isVisible) {
            clearFlag(window, UNSTABLE_STATUS);
        } else {
