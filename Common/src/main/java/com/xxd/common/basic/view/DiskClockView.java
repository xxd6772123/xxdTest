...
public class DiskClockView extends View {
    ...
    public DiskClockView(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.mContext = context;
        init(context, attrs);
        initPainter();
    ...
    private void init(Context context, AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.DiskClockView);
        borderColor = ta.getColor(R.styleable.DiskClockView_borderColor, Color.BLACK);
        circleBackground = ta.getColor(R.styleable.DiskClockView_circleBackground, Color.WHITE);
        borderWidth = ta.getDimension(R.styleable.DiskClockView_borderWidth,
                SizeUtils.dp2px(context, 1));
        minScaleColor = ta.getColor(R.styleable.DiskClockView_minScaleColor, Color.BLACK);
        midScaleColor = ta.getColor(R.styleable.DiskClockView_midScaleColor, Color.BLACK);
        maxScaleColor = ta.getColor(R.styleable.DiskClockView_maxScaleColor, Color.BLACK);
        minScaleLength = ta.getDimension(R.styleable.DiskClockView_minScaleLength,
                SizeUtils.dp2px(context, 7));
        midScaleLength = ta.getDimension(R.styleable.DiskClockView_midScaleLength,
                SizeUtils.dp2px(context, 12));
        maxScaleLength = ta.getDimension(R.styleable.DiskClockView_maxScaleLength,
                SizeUtils.dp2px(context, 14));
        textColor = ta.getColor(R.styleable.DiskClockView_textColor, Color.BLACK);
        textSize = ta.getDimension(R.styleable.DiskClockView_textSize,
                SizeUtils.dp2px(context, 15));
        isDrawText = ta.getBoolean(R.styleable.DiskClockView_isDrawText, true);
        secondPointerColor = ta.getColor(R.styleable.DiskClockView_secondPointerColor, Color.RED);
        minPointerColor = ta.getColor(R.styleable.DiskClockView_minPointerColor, Color.BLACK);
        hourPointerColor = ta.getColor(R.styleable.DiskClockView_hourPointerColor, Color.BLACK);
        secondPointerLength = ta.getDimension(R.styleable.DiskClockView_secondPointerLength,
                SizeUtils.dp2px(context, getWidth() / 3 * 2 / 3));
        minPointerLength = ta.getDimension(R.styleable.DiskClockView_minPointerLength,
                SizeUtils.dp2px(context, getWidth() / 3 / 2));
        hourPointerLength = ta.getDimension(R.styleable.DiskClockView_hourPointerLength,
                SizeUtils.dp2px(context, getWidth() / 3 / 3));
        secondPointerSize = ta.getDimension(R.styleable.DiskClockView_secondPointerSize,
                SizeUtils.dp2px(context, 1));
        minPointerSize = ta.getDimension(R.styleable.DiskClockView_minPointerSize,
                SizeUtils.dp2px(context, 3));
        hourPointerSize = ta.getDimension(R.styleable.DiskClockView_hourPointerSize,
                SizeUtils.dp2px(context, 5));
        centerPointColor = ta.getColor(R.styleable.DiskClockView_centerPointColor, Color.BLACK);
        centerPointSize = ta.getDimension(R.styleable.DiskClockView_centerPointSize,
                SizeUtils.dp2px(context, 5));
        centerPointRadiu = ta.getDimension(R.styleable.DiskClockView_centerPointRadiu,
                SizeUtils.dp2px(context, 1));
        centerPointType = ta.getString(R.styleable.DiskClockView_centerPointType);
        isSecondGoSmooth = ta.getBoolean(R.styleable.DiskClockView_isSecondGoSmooth, false);
        if (isSecondGoSmooth) {
            sleepTime = 50;
        } else {
            sleepTime = 1000;
